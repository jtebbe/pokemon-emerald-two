...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_OCCULTIST] = {_("AGARTHAN OCCULTIST")},
    [TRAINER_CLASS_KING] = {_("AGARTHAN KING")},
    [TRAINER_CLASS_SCRIBE] = {_("AGARTHAN SCRIBE")},
    [TRAINER_CLASS_SLAYER] = {_("AGARTHAN SLAYER")},
    [TRAINER_CLASS_JESTER] = {_("AGARTHAN JESTER")},
    [TRAINER_CLASS_DOCTOR] = {_("AGARTHAN DOCTOR")},
    [TRAINER_CLASS_AGARTHAN_PSYCHIC] = {_("AGARTHAN PSYCHIC")},
    [TRAINER_CLASS_SCOUT] = {_("AGARTHAN SCOUT")},
    [TRAINER_CLASS_SAGE] = {_("AGARTHAN SAGE")},
};
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            CreateMon(&party[i], partyData[monIndex].species, GetHighestLevelInPlayerParty(), 0, TRUE, personalityValue, otIdType, fixedOtId);
            ...
            if (FlagGet(FLAG_NO_EVS_MODE)) {
                u8 fixedEv = 0;
                SetMonData(&party[i], MON_DATA_HP_EV, &fixedEv);
                SetMonData(&party[i], MON_DATA_ATK_EV, &fixedEv);
                SetMonData(&party[i], MON_DATA_DEF_EV, &fixedEv);
                SetMonData(&party[i], MON_DATA_SPATK_EV, &fixedEv);
                SetMonData(&party[i], MON_DATA_SPDEF_EV, &fixedEv);
                SetMonData(&party[i], MON_DATA_SPEED_EV, &fixedEv);
            }
            else if (partyData[monIndex].ev != NULL)
            {
...
u32 GetBattlerTotalSpeedStat(u32 battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            speed *= 2;
        else if ((ability == ABILITY_CHLOROPHYLL || ability == ABILITY_CHLOROTOXIN) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_SUN)
            speed *= 2;
...
enum Type GetDynamicMoveType(struct Pokemon *mon, u32 move, u32 battler, enum MonState state)
{
    ...
    case EFFECT_TERRAIN_PULSE:
        if (state == MON_IN_BATTLE)
        {
            if (GetBattlerAbility(battler) == ABILITY_STORMBREAKER)
