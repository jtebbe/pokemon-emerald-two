...
bool8 CheckForTrainersWantingBattle(void)
{
    ...
    for (i = 0; i <= trainerObjectsCount; i++)
    {
        ...
        if (gNoOfApproachingTrainers > 1)
            break;
        if (GetMonsStateToDoubles_2(FALSE) != PLAYER_HAS_TWO_USABLE_MONS) // one trainer found and cant have a double battle
            break;
    }
...
static u8 CheckTrainer(u8 objectEventId)
{
    ...
    if (trainerBattlePtr)
    {
        ...
        {
            if (GetMonsStateToDoubles_2(FALSE) != PLAYER_HAS_TWO_USABLE_MONS)
                return 0;
