...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_MOON_BALL,       {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    { ITEM_CHERISH_BALL,    {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    ...
    { ITEM_EXP_CANDY_XL,    {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_PROTEIN,         {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_ZINC,            {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_CARBOS,          {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_IRON,            {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_CALCIUM,         {   _,   _,   1,   4,   4,   4,   4,   5,   4,   5, } },
...
static void Cmd_attackcanceler(void)
{
    ...
    if (gSpecialStatuses[gBattlerAttacker].oneTwoState == ONE_TWO_OFF
     && GetBattlerAbility(gBattlerAttacker) == ABILITY_ONE_TWO
     && IsMoveAffectedByOneTwo(gCurrentMove, gBattlerAttacker)
     && !(gAbsentBattlerFlags & (1u << gBattlerTarget))
     && GetActiveGimmick(gBattlerAttacker) != GIMMICK_Z_MOVE)
    {
        gSpecialStatuses[gBattlerAttacker].oneTwoState = ONE_TWO_1ST_HIT;
        gMultiHitCounter = 2;
        PREPARE_BYTE_NUMBER_BUFFER(gBattleScripting.multihitString, 1, 0)
        return;
    }
     ...
     && moveEffect != EFFECT_COUNTER)
    {
        ...
        if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_1ST_HIT)
        {
            ...
            gMultiHitCounter = 0;
        } else if (gSpecialStatuses[gBattlerAttacker].oneTwoState == ONE_TWO_1ST_HIT)
        {
            gSpecialStatuses[gBattlerAttacker].oneTwoState = ONE_TWO_OFF; // No second hit if first hit was blocked
            gSpecialStatuses[gBattlerAttacker].multiHitOn = 0;
            gMultiHitCounter = 0;
        }
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_2ND_HIT
        || gSpecialStatuses[gBattlerAttacker].oneTwoState == ONE_TWO_2ND_HIT
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Ability abilityAtk, enum Ability abilityDef, enum HoldEffect holdEffectAtk)
{
    ...
    if (gSideStatuses[GetBattlerSide(battlerDef)] & SIDE_STATUS_LUCKY_CHANT)
    {
        critChance = CRITICAL_HIT_BLOCKED;
          ...
          || abilityAtk == ABILITY_SOUL_STRIKES)
    {
...
static void Cmd_attackanimation(void)
{
    ...
    {
        if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_2ND_HIT || gSpecialStatuses[gBattlerAttacker].oneTwoState == ONE_TWO_2ND_HIT) // No animation on second hit
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
            return;
        }
...
static inline bool32 TrySetReflect(u32 battler)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_REFLECT))
    {
        ...
        else if (GetBattlerHoldEffect(battler) == HOLD_EFFECT_LIGHT_CLAY)
    ...
    case MOVE_EFFECT_WRAP:
        ...
        else
        {
            if (GetBattlerHoldEffect(gBattlerAttacker) == HOLD_EFFECT_GRIP_CLAW)
                gDisableStructs[gEffectBattler].wrapTurns = GetConfig(CONFIG_BINDING_TURNS) >= GEN_5 ? 10 : 5;
            else
    ...
    case MOVE_EFFECT_FIRE_SPIN_SIDE:
    {
        ...
        for (battler = 0; battler < MAX_BATTLERS_COUNT; ++battler)
        {
            ...
            if (!gBattleMons[battler].volatiles.wrapped)
            {
                ...
                if (GetBattlerHoldEffect(gBattlerAttacker) == HOLD_EFFECT_GRIP_CLAW)
                    gDisableStructs[battler].wrapTurns = (GetConfig(CONFIG_BINDING_TURNS) >= GEN_5) ? 10 : 5;
                else
    ...
    case ABILITY_BEAST_BOOST:
        {
            ...
            else if (abilityAtk == ABILITY_GRIM_NEIGH || abilityAtk == ABILITY_AS_ONE_SHADOW_RIDER || abilityAtk == ABILITY_TRIUMPHANCE)
                stat = STAT_SPATK;
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_MULTIHIT_MOVE:
        {
            ...
            gSpecialStatuses[gBattlerAttacker].oneTwoState = ONE_TWO_OFF;
            gSpecialStatuses[gBattlerAttacker].multiHitOn = 0;
            gBattleScripting.moveendState++;
            break;
        }
...
static void Cmd_trysetrest(void)
{
    ...
    if (IsBattlerTerrainAffected(gBattlerTarget, ability, holdEffect, STATUS_FIELD_ELECTRIC_TERRAIN) || GetBattlerAbility(gBattlerTarget) == ABILITY_STORMBREAKER)
    {
...
void BS_RemoveStockpileCounters(void)
{
    ...
    if (GetMoveEffect(gCurrentMove) == EFFECT_SPIT_UP
     && (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_1ST_HIT || gSpecialStatuses[gBattlerAttacker].oneTwoState == ONE_TWO_1ST_HIT)
     && IsBattlerAlive(gBattlerTarget))
    {
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static u32 ChangeStatBuffs(u32 battler, s8 statValue, enum Stat statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    else if ((battlerAbility == ABILITY_SIMPLE || battlerAbility == ABILITY_CONTRARIAN) && !flags.onlyChecking)
    {
...
static void Cmd_setmist(void)
{
    ...
    {
        if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TEMPORAL_ASSERTION) {
            gSideTimers[GetBattlerSide(gBattlerAttacker)].mistTimer = 250;
        } else {
...
static void Cmd_presentdamagecalculation(void)
{
     ...
     */
    if (gSpecialStatuses[gBattlerAttacker].parentalBondState != PARENTAL_BOND_2ND_HIT && gSpecialStatuses[gBattlerAttacker].oneTwoState != ONE_TWO_2ND_HIT)
    {
        if (rand < 102)
        {
...
static void Cmd_setsafeguard(void)
{
    ...
    else
    {
        ...
        if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TEMPORAL_ASSERTION) {
            gSideTimers[GetBattlerSide(gBattlerAttacker)].safeguardTimer = 250;
        } else {
...
static bool32 CheckIfCanFireTwoTurnMoveNow(u8 battler, bool8 checkChargeTurnEffects)
{
    ...
    if ((gCurrentMove == MOVE_FLY || gCurrentMove == MOVE_SKY_ATTACK) && gSideStatuses[GetBattlerSide(gBattlerAttacker)] & SIDE_STATUS_TAILWIND)
        return TRUE;
    ...
    if (GetAttackerWeather(GetBattlerHoldEffect(gBattlerAttacker), GetBattlerAbility(gBattlerAttacker), GetWeather()) & GetMoveTwoTurnAttackWeather(gCurrentMove))
        return TRUE;
...
static void Cmd_setyawn(void)
{
    ...
    else if (IsBattlerTerrainAffected(gBattlerTarget, ability, holdEffect, STATUS_FIELD_ELECTRIC_TERRAIN) || GetBattlerAbility(gBattlerTarget) == ABILITY_STORMBREAKER)
    {
        ...
        if (GetMoveEffect(gCurrentMove) == EFFECT_MUD_SPORT)
        {
            if (B_SPORT_TURNS >= GEN_6)
            {
                if (!(gFieldStatuses & STATUS_FIELD_MUDSPORT))
                {
                    ...
                    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TEMPORAL_ASSERTION) {
                        gFieldTimers.mudSportTimer = 250;
                    } else {
        ...
        else // Water Sport
        {
            if (B_SPORT_TURNS >= GEN_6)
            {
                if (!(gFieldStatuses & STATUS_FIELD_WATERSPORT))
                {
                    ...
                    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TEMPORAL_ASSERTION) {
                        gFieldTimers.waterSportTimer = 250;
                    } else {
...
bool32 DoesSubstituteBlockMove(u32 battlerAtk, u32 battlerDef, u32 move)
{
    if (!gBattleMons[battlerDef].volatiles.substitute)
        return FALSE;
    else if (GetBattlerHoldEffect(battlerAtk) == HOLD_EFFECT_BURGLARY_KIT)
        return FALSE;
    ...
    case GIVECAUGHTMON_GIVE_AND_SHOW_MSG:
    {
        ...
        if (B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9)
        {
            ...
            if (lostItem != ITEM_NONE)
                SetMonData(caughtMon, MON_DATA_HELD_ITEM, &lostItem);  // Restore items
        }
...
void BS_SetPledgeStatus(void)
{
    ...
    if (!(gSideStatuses[side] & cmd->sideStatus))
    {
        ...
        case SIDE_STATUS_RAINBOW:
            if (GetBattlerAbility(battler) == ABILITY_TEMPORAL_ASSERTION) {
                gSideTimers[side].rainbowTimer = 250;
            } else {
        ...
        if (GetBattlerAbility(battler) == ABILITY_TEMPORAL_ASSERTION) {
                gSideTimers[side].seaOfFireTimer = 250;
            } else {
        ...
        if (GetBattlerAbility(battler) == ABILITY_TEMPORAL_ASSERTION) {
                gSideTimers[side].swampTimer = 250;
            } else {
...
void BS_TryActivateGulpMissile(void)
{
        ...
        && GetBattlerAbility(gBattlerTarget) == ABILITY_GULP_MISSILE)
    {
        if (!IsAbilityAndRecord(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), ABILITY_MAGIC_GUARD))
            SetPassiveDamageAmount(gBattlerTarget, GetNonDynamaxMaxHP(gBattlerAttacker) / 3);
...
void BS_TrySpectralThiefSteal(void)
{
    ...
    bool32 contrary = (GetBattlerAbility(gBattlerAttacker) == ABILITY_CONTRARY || GetBattlerAbility(gBattlerAttacker) == ABILITY_CONTRARIAN);
...
void BS_TryActivateEternalPromise(void)
{
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battler))
        && GetBattlerHoldEffect(BATTLE_PARTNER(battler)) == HOLD_EFFECT_ETERNAL_PROMISE)
    {
