...
static enum ItemEffect TryKingsRock(u32 battlerAtk, u32 battlerDef, u32 item)
{
    ...
    if (B_SERENE_GRACE_BOOST >= GEN_5 && (ability == ABILITY_SERENE_GRACE || ability == ABILITY_MAGIC_WINDS))
        holdEffectParam *= 2;
...
static enum ItemEffect TryStrangeSword(u32 battler, ActivationTiming timing) {
    enum ItemEffect effect = ITEM_NO_EFFECT;
    gSpecialStatuses[battler].switchInItemDone = TRUE;
    if (timing == IsOnSwitchInFirstTurnActivation)
        BattleScriptPushCursorAndCallback(BattleScript_StrangeSwordHeld);
    else
        BattleScriptCall(BattleScript_StrangeSwordHeldRet);
    RecordItemEffectBattle(battler, HOLD_EFFECT_STRANGE_SWORD);
    effect = ITEM_EFFECT_OTHER;
    return effect;
}

static enum ItemEffect TryStrangeShield(u32 battler, ActivationTiming timing) {
    enum ItemEffect effect = ITEM_NO_EFFECT;
    gSpecialStatuses[battler].switchInItemDone = TRUE;
    if (timing == IsOnSwitchInFirstTurnActivation)
        BattleScriptPushCursorAndCallback(BattleScript_StrangeShieldHeld);
    else
        BattleScriptCall(BattleScript_StrangeShieldHeldRet);
...
static enum ItemEffect TrySetEnigmaBerry(u32 battlerDef, u32 battlerAtk)
{
    ...
    {
        s32 healAmount = gBattleMons[battlerDef].maxHP;
        if (GetBattlerAbility(battlerDef) == ABILITY_RIPEN)
            healAmount *= 2;
...
enum ItemEffect ItemBattleEffects(u32 itemBattler, u32 battler, enum HoldEffect holdEffect, ActivationTiming timing)
{
    ...
    case HOLD_EFFECT_STRANGE_SWORD:
        effect = TryStrangeSword(itemBattler, timing);
        break;
    case HOLD_EFFECT_STRANGE_SHIELD:
        effect = TryStrangeShield(itemBattler, timing);
        break;
