...
static enum MoveCanceler CancelerRagebaited(void)
{
...
bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag)
{
    ...
    if (!(gFieldStatuses & statusFlag))
    {
        ...
        else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TEMPORAL_ASSERTION)
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_GULP_MISSILE:
             ...
             && gBattleMons[gBattlerTarget].species != SPECIES_CRAMORANT)
            {
                ...
                {
                    gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / 3;
                    if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
                        gBattleStruct->moveDamage[gBattlerAttacker] = 1;
                }
        ...
        if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_LUMINOUS_MOSS:
                    ...
                    && moveType == TYPE_WATER)
                {
                    effect = ITEM_STATS_CHANGE;
                    BattleScriptCall(BattleScript_TargetItemStatRaise);
                    SET_STATCHANGER(STAT_SPDEF, 3, FALSE);
                }
                break;
...
static inline uq4_12_t GetAttackerItemsModifier(u32 battlerAtk, uq4_12_t typeEffectivenessModifier, enum ItemHoldEffect holdEffectAtk, u32 isCrit)
{
    u32 metronomeTurns;
