...
void HandleAction_UseMove(void)
{
    ...
    {
        gBattleStruct->categoryOverride = GetMoveCategory(gCurrentMove);
        gCurrentMove = GetMaxMove(gBattlerAttacker, gCurrentMove);
        if (gBattleStruct->thesaurusMove[gBattlerAttacker] == MOVE_NONE)
            gChosenMove = gCurrentMove;
    }
...
bool32 TryRunFromBattle(u32 battler)
{
    ...
    else if (CanGhostTypeAlwaysEscape(battler))
    {
        effect++;
    }
...
void PrepareStringBattle(enum StringID stringId, u32 battler)
{
    ...
    case STRINGID_STATSWONTINCREASE2:
        if (battlerAbility == ABILITY_CONTRARY || battlerAbility == ABILITY_CONTRARIAN)
    ...
    case STRINGID_PKMNCUTSATTACKWITH:
        ...
        else if (targetAbility == ABILITY_CONTRARY || targetAbility == ABILITY_CONTRARIAN)
        {
...
static enum MoveCanceler CancelerRagebaited(struct BattleContext *ctx)
{
    if (GetActiveGimmick(ctx->battlerAtk) != GIMMICK_Z_MOVE && IsAbilityOnField(ABILITY_RAGEBAITER) && IsBattleMoveStatus(ctx->currentMove) && GetBattlerAbility(ctx->battlerAtk) != ABILITY_OBLIVIOUS)
    {
...
static enum MoveCanceler CancelerPaintbrush(struct BattleContext *ctx)
{
    enum Type moveType = GetBattleMoveType(ctx->currentMove);
    enum Type oldPrimaryType = gBattleMons[ctx->battlerAtk].types[0];
    enum Type oldSecondaryType = gBattleMons[ctx->battlerAtk].types[1];
    const u8 *battleScript;

    if (GetBattlerHoldEffect(ctx->battlerAtk) != HOLD_EFFECT_PAINTBRUSH
     || gDisableStructs[ctx->battlerAtk].usedPaintbrush
     || moveType == TYPE_MYSTERY
     || moveType == TYPE_NONE
     || ctx->currentMove == MOVE_STRUGGLE
     || IS_BATTLER_OF_TYPE(ctx->battlerAtk, moveType))
        return MOVE_STEP_SUCCESS;

    if (oldSecondaryType == TYPE_NONE || oldSecondaryType == TYPE_MYSTERY || oldSecondaryType == oldPrimaryType)
    {
        battleScript = BattleScript_PaintbrushAddsType;
    }
    else
    {
        PREPARE_TYPE_BUFFER(gBattleTextBuff1, oldSecondaryType);
        battleScript = BattleScript_PaintbrushReplacesType;
    }
    PREPARE_TYPE_BUFFER(gBattleTextBuff2, moveType);
    gBattleMons[ctx->battlerAtk].types[1] = moveType;
    gDisableStructs[ctx->battlerAtk].usedPaintbrush = TRUE;
    RecordItemEffectBattle(ctx->battlerAtk, HOLD_EFFECT_PAINTBRUSH);
    BattleScriptCall(battleScript);
    return MOVE_STEP_BREAK;
}
...
static enum MoveCanceler (*const sMoveSuccessOrderCancelers[])(struct BattleContext *ctx) =
{
    ...
    [CANCELLER_RAGEBAITED] = CancelerRagebaited,
    ...
    [CANCELER_PAINTBRUSH] = CancelerPaintbrush,
...
bool32 HadMoreThanFifthHpNowDoesnt(u32 battler)
{
    // Had more than half of hp before, now has less
     return gBattleStruct->battlerState[battler].wasAboveHalfHp
        && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 5;
}
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SPACIAL_DOMINANCE:
            ...
            break;
        case ABILITY_HALLOWS_END:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_HALLOWS_END;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
            break;
        case ABILITY_TEMPORAL_ASSERTION:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_TEMPORAL_ASSERTION;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
        ...
        case ABILITY_SUPERSWEET_SYRUP:
            ...
            break;
        case ABILITY_MAGIC_WINDS:
            u8 side = GetBattlerSide(battler);
            if (!(gSideStatuses[side] & SIDE_STATUS_TAILWIND) && !gSpecialStatuses[battler].switchInAbilityDone) {
                gSideStatuses[side] |= SIDE_STATUS_TAILWIND;
                gSideTimers[side].tailwindTimer = 2;
                BattleScriptPushCursorAndCallback(BattleScript_MagicWindsActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN:
        ...
        case ABILITY_HARVEST:
            ...
            case ABILITY_ICE_BODY:
                if (IsBattlerWeatherAffected(GetBattlerHoldEffect(battler), GetWeather(), B_WEATHER_HAIL | B_WEATHER_SNOW)
            ...
            case ABILITY_POWER_HOARD:
                if (CompareStat(battler, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility) && gDisableStructs[battler].isFirstTurn != 2)
                {
                    SaveBattlerAttacker(gBattlerAttacker);
                    SET_STATCHANGER(STAT_CHANGE_SPATK, 1, FALSE);
                    BattleScriptExecute(BattleScript_AttackerAbilityStatRaiseEnd2);
                    gBattleScripting.battler = battler;
                    effect++;
                }
            ...
            case ABILITY_MIND_ETERNAL:
                if (CompareStat(battler, STAT_SPDEF, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility) && gDisableStructs[battler].isFirstTurn != 2)
                {
                    SaveBattlerAttacker(gBattlerAttacker);
                    SET_STATCHANGER(STAT_CHANGE_SPDEF, 1, FALSE);
                    BattleScriptExecute(BattleScript_AttackerAbilityStatRaiseEnd2);
                    gBattleScripting.battler = battler;
                    effect++;
                }
        ...
        case ABILITY_SNOW_BLOW:
             ...
             && !(gBattleWeather & B_WEATHER_SNOW && HasWeatherEffect()))
            {
                ...
                else if (TryChangeBattleWeather(battler, BATTLE_WEATHER_SNOW, TRUE))
                {
                    gBattleScripting.battler = battler;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_SnowBlowActivates;
                    effect++;
                }
            }
            break;
    ...
    case ABILITYEFFECT_ON_WEATHER: // For ability effects that activate when the battle weather changes.
        ...
        case ABILITY_ICE_FACE:
        {
            ...
            u32 battlerWeatherAffected = IsBattlerWeatherAffected(GetBattlerHoldEffect(battler), weather, weather);
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (ability == ABILITY_SHADOW_TAG && IsBattlerAlive(battlerDef) && (B_SHADOW_TAG_ESCAPE <= GEN_3 || GetBattlerAbility(battler) != ABILITY_SHADOW_TAG))
            return battlerDef + 1;
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_TOXIC:
        ...
        else if ((abilityAtk != ABILITY_CORROSION && abilityAtk != ABILITY_CHLOROTOXIN) && IS_BATTLER_ANY_TYPE(battlerDef, TYPE_POISON, TYPE_STEEL))
        {
    ...
    case MOVE_EFFECT_SLEEP:
        ...
        else if (IsBattlerTerrainAffected(battlerDef, abilityDef, GetBattlerHoldEffect(battlerDef), STATUS_FIELD_ELECTRIC_TERRAIN) || abilityDef == ABILITY_STORMBREAKER)
        {
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (GetBattlerAbility(battlerAtk) == ABILITY_LORD_OF_SPACE
        || GetBattlerHoldEffect(battlerAtk) == HOLD_EFFECT_BURGLARY_KIT) {
        return FALSE;
    }
    ...
    if (gProtectStructs[battlerDef].protected != PROTECT_MAX_GUARD && !MoveIgnoresProtect(move))
    {
        ...
        if ((GetBattlerAbility(battlerAtk) == ABILITY_UNSEEN_FIST || GetBattlerAbility(battlerAtk) == ABILITY_PIERCING_DRILL)
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_MISTY_EXPLOSION:
        if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_MISTY_TERRAIN))
            basePower = uq4_12_multiply(basePower, UQ_4_12(3.0));
        break;
    ...
    case EFFECT_RISING_VOLTAGE:
        if (IsBattlerTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, STATUS_FIELD_ELECTRIC_TERRAIN) || ctx->abilityAtk == ABILITY_STORMBREAKER)
            basePower *= 2;
        break;
    case EFFECT_BEAT_UP:
        if (GetConfig(CONFIG_BEAT_UP) >= GEN_5)
    ...
    case EFFECT_PSYBLADE:
        if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || ctx->abilityAtk == ABILITY_STORMBREAKER)
            basePower = uq4_12_multiply(basePower, UQ_4_12(1.5));
        break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case EFFECT_EARTHQUAKE:
        if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN && !IsSemiInvulnerable(battlerDef, CHECK_ALL))
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        if (!IsSemiInvulnerable(battlerDef, CHECK_ALL) && GetBattlerHoldEffect(battlerDef) == HOLD_EFFECT_FLOAT_STONE)
    ...
    if (IsBattlerTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, STATUS_FIELD_MISTY_TERRAIN) && moveType == TYPE_DRAGON)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if ((IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_ELECTRIC_TERRAIN) || ctx->abilityAtk == ABILITY_STORMBREAKER) && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    case ABILITY_FLARE_BOOST:
        if (gBattleMons[battlerAtk].status1 & STATUS1_BURN && IsBattlerBattleMoveSpecial(battlerAtk, move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
        break;
    case ABILITY_TOXIC_BOOST:
        if (gBattleMons[battlerAtk].status1 & STATUS1_PSN_ANY && IsBattlerBattleMovePhysical(battlerAtk, move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        case ABILITY_BATTERY:
            if (IsBattlerBattleMoveSpecial(battlerAtk, move))
                modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
            break;
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if (((ctx->weather & B_WEATHER_SUN && HasWeatherEffect()) || gDisableStructs[battlerDef].boosterEnergyActivated)
             && ((IsBattlerBattleMovePhysical(battlerAtk, move) && defHighestStat == STAT_DEF) || (IsBattlerBattleMoveSpecial(battlerAtk, move) && defHighestStat == STAT_SPDEF))
    ...
    case HOLD_EFFECT_MUSCLE_BAND:
        if (IsBattlerBattleMovePhysical(battlerAtk, move))
    ...
    case HOLD_EFFECT_SOUL_DEW:
        if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS)
            && ((B_SOUL_DEW_BOOST >= GEN_7 && (moveType == TYPE_PSYCHIC || moveType == TYPE_DRAGON))
             || (B_SOUL_DEW_BOOST < GEN_7 && !(gBattleTypeFlags & BATTLE_TYPE_FRONTIER) && IsBattlerBattleMoveSpecial(battlerAtk, move))))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
...
{
    if (ShouldGetStatBadgeBoost(B_FLAG_BADGE_BOOST_ATTACK, battler) && IsBattlerBattleMovePhysical(battler, move))
        modifier = uq4_12_multiply_half_down(modifier, GetBadgeBoostModifier());
    if (ShouldGetStatBadgeBoost(B_FLAG_BADGE_BOOST_SPATK, battler) && IsBattlerBattleMoveSpecial(battler, move))
        modifier = uq4_12_multiply_half_down(modifier, GetBadgeBoostModifier());
    return modifier;
}

static inline uq4_12_t ApplyDefensiveBadgeBoost(uq4_12_t modifier, u32 battler, u32 battlerAtk, u32 move)
{
    if (ShouldGetStatBadgeBoost(B_FLAG_BADGE_BOOST_DEFENSE, battler) && IsBattlerBattleMovePhysical(battlerAtk, move))
        modifier = uq4_12_multiply_half_down(modifier, GetBadgeBoostModifier());
    if (ShouldGetStatBadgeBoost(B_FLAG_BADGE_BOOST_SPDEF, battler) && IsBattlerBattleMoveSpecial(battlerAtk, move))
        modifier = uq4_12_multiply_half_down(modifier, GetBadgeBoostModifier());
    return modifier;
}

static inline enum Stat GetPuzzleBoxDamageStat(enum HoldEffect holdEffect, enum Stat statId)
{
    if (holdEffect != HOLD_EFFECT_PUZZLE_BOX)
        return statId;
...
static inline u32 GetBattlerDamageStat(u32 battler, enum Stat statId)
{
    switch (statId)
    {
    case STAT_ATK:
        return gBattleMons[battler].attack;
    case STAT_DEF:
        return gBattleMons[battler].defense;
    case STAT_SPATK:
        return gBattleMons[battler].spAttack;
    case STAT_SPDEF:
        return gBattleMons[battler].spDefense;
    default:
        return 1;
    }
}
...
{
    enum Stat atkStatId;
    enum Stat atkStageId;
    enum HoldEffect holdEffectStatSource;
    u8 atkStage;
    u32 atkStat;
    uq4_12_t modifier;
    ...
    u32 battlerAtk = ctx->battlerAtk;
    u32 battlerDef = ctx->battlerDef;
    u32 battlerStatSource = battlerAtk;
    u32 move = ctx->move;
    enum Type moveType = ctx->moveType;
    enum BattleMoveEffects moveEffect = GetMoveEffect(move);
    bool32 isPhysical = IsBattlerBattleMovePhysical(battlerAtk, move);
    bool32 isSpecial = IsBattlerBattleMoveSpecial(battlerAtk, move);

    atkBaseSpeciesId = GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species);
    holdEffectStatSource = ctx->holdEffectAtk;

    if (moveEffect == EFFECT_FOUL_PLAY)
    {
        battlerStatSource = battlerDef;
        holdEffectStatSource = ctx->holdEffectDef;
        if (isPhysical)
        {
            atkStatId = STAT_ATK;
            atkStageId = STAT_ATK;
        ...
        {
            atkStatId = STAT_SPATK;
            atkStageId = STAT_SPATK;
        }
    }
    else if (moveEffect == EFFECT_BODY_PRESS)
    {
        if (isPhysical)
        {
            ...
            if (gFieldStatuses & STATUS_FIELD_WONDER_ROOM)
                atkStageId = STAT_SPDEF;
            else
                atkStageId = STAT_DEF;
        }
        else
        {
            atkStatId = STAT_SPDEF;
            atkStageId = STAT_SPDEF;
        }
    }
    else
    ...
    atkStatId = GetPuzzleBoxDamageStat(holdEffectStatSource, atkStatId);
    atkStageId = GetPuzzleBoxDamageStat(holdEffectStatSource, atkStageId);
    atkStat = GetBattlerDamageStat(battlerStatSource, atkStatId);
    atkStage = gBattleMons[battlerStatSource].statStages[atkStageId];
    ...
    case ABILITY_SLOW_START:
        ...
        break;
    case ABILITY_SOLAR_POWER:
        if (isSpecial && IsBattlerWeatherAffected(ctx->holdEffectAtk, ctx->weather, B_WEATHER_SUN))
    ...
    case ABILITY_STICK_HOARDER:
        ...
        if (GetItemHoldEffect(battlerAtk) != HOLD_EFFECT_LEEK && gBattleStruct->itemLost[side][gBattlerPartyIndexes[battlerAtk]].originalItem == ITEM_LEEK && !isSpecial)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case ABILITY_PLUS:
        if (isSpecial && IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
        {
    ...
    case ABILITY_MINUS:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[battlerAtk].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(ctx->holdEffectAtk, ctx->weather, B_WEATHER_SUN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (isPhysical)
    ...
    case ABILITY_STAKEOUT:
        ...
        break;
    case ABILITY_GUTS:
        if (gBattleMons[battlerAtk].status1 & STATUS1_ANY && isPhysical)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case ABILITY_PROTOSYNTHESIS:
        if (!(gBattleMons[battlerAtk].volatiles.transformed))
        {
            ...
            {
                if ((isPhysical && atkHighestStat == STAT_ATK) || (isSpecial && atkHighestStat == STAT_SPATK))
                    modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
            }
        }
        break;
    case ABILITY_QUARK_DRIVE:
        ...
        break;
    case ABILITY_ORICHALCUM_PULSE:
        if ((ctx->weather & B_WEATHER_SUN) && HasWeatherEffect() && isPhysical
            && ctx->holdEffectAtk != HOLD_EFFECT_UTILITY_UMBRELLA)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        break;
    case ABILITY_HADRON_ENGINE:
        if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN && isSpecial)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        break;
    default:
        break;
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(GetBattlerHoldEffect(BATTLE_PARTNER(battlerAtk)), ctx->weather, B_WEATHER_SUN))
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
    ...
    // Ruin field effects
    if (isSpecial && !gBattleMons[ctx->battlerAtk].volatiles.vesselOfRuin && IsRuinStatusActive(VOLATILE_VESSEL_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (isPhysical && !gBattleMons[ctx->battlerAtk].volatiles.tabletsOfRuin && IsRuinStatusActive(VOLATILE_TABLETS_OF_RUIN))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_CUBONE || atkBaseSpeciesId == SPECIES_MAROWAK) && isPhysical)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    ...
    case HOLD_EFFECT_DEEP_SEA_TOOTH:
        if (gBattleMons[battlerAtk].species == SPECIES_CLAMPERL && isSpecial)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_LIGHT_BALL:
        if (atkBaseSpeciesId == SPECIES_PIKACHU && (GetConfig(CONFIG_LIGHT_BALL_ATTACK_BOOST) >= GEN_4 || isSpecial))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_CHOICE_BAND:
        if (isPhysical && GetActiveGimmick(battlerAtk) != GIMMICK_DYNAMAX)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case HOLD_EFFECT_CHOICE_SPECS:
        if (isSpecial && GetActiveGimmick(battlerAtk) != GIMMICK_DYNAMAX)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    default:
        break;
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (moveEffect == EFFECT_PSYSHOCK || isPhysical) // uses defense stat instead of sp.def
    {
        if (gFieldStatuses & STATUS_FIELD_WONDER_ROOM) // the defense stats are swapped
            defStatId = STAT_SPDEF;
        else
            defStatId = STAT_DEF;
        defStageId = STAT_DEF;
    }
    else // is special
    {
        if (gFieldStatuses & STATUS_FIELD_WONDER_ROOM) // the defense stats are swapped
            ...
            defStatId = STAT_SPDEF;
        defStageId = STAT_SPDEF;
    }

    defStatId = GetPuzzleBoxDamageStat(ctx->holdEffectDef, defStatId);
    defStageId = GetPuzzleBoxDamageStat(ctx->holdEffectDef, defStageId);
    defStat = GetBattlerDamageStat(battlerDef, defStatId);
    defStage = gBattleMons[battlerDef].statStages[defStageId];
    usesDefStat = defStatId == STAT_DEF;
    ...
    if (MoveIgnoresDefenseEvasionStages(move))
        defStage = DEFAULT_STAT_STAGE;
    // Crowbar ignores only the target's raised defensive stages for contact attacks.
    if (ctx->holdEffectAtk == HOLD_EFFECT_CROWBAR
     && defStage > DEFAULT_STAT_STAGE
     && IsMoveMakingContact(ctx->battlerAtk, battlerDef, ctx->abilityAtk, ctx->holdEffectAtk, move))
        defStage = DEFAULT_STAT_STAGE;
    ...
    if (ctx->isSelfInflicted)
        return uq4_12_multiply_by_int_half_down(ApplyDefensiveBadgeBoost(modifier, battlerDef, battlerAtk, move), defStat);
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
        ...
        /*
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerDef)].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(GetBattlerHoldEffect(BATTLE_PARTNER(battlerDef)), ctx->weather, B_WEATHER_SUN) && !usesDefStat)
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        */
        default:
    ...
    if (GetConfig(CONFIG_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_ROCK) && GetAttackerWeather(ctx->holdEffectAtk, ctx->abilityAtk, ctx->weather) & B_WEATHER_SANDSTORM && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // snow def boost for ice types
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && GetAttackerWeather(ctx->holdEffectAtk, ctx->abilityAtk, ctx->weather) & B_WEATHER_SNOW && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));

    modifier = ApplyDefensiveBadgeBoost(modifier, battlerDef, battlerAtk, move);
...
static inline uq4_12_t GetOneTwoModifier(u32 battlerAtk)
{
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_BURN
        && IsBattlerBattleMovePhysical(ctx->battlerAtk, ctx->move)
    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_FROSTBITE
        && IsBattlerBattleMoveSpecial(ctx->battlerAtk, ctx->move)
        && (GetConfig(CONFIG_BURN_FACADE_DMG) < GEN_6 || moveEffect != EFFECT_FACADE))
        return UQ_4_12(0.5);
    return UQ_4_12(1.0);
}
...
static inline uq4_12_t GetScreensModifier(struct DamageContext *ctx)
{
    ...
    bool32 lightScreen = (sideStatus & SIDE_STATUS_LIGHTSCREEN) && IsBattlerBattleMoveSpecial(ctx->battlerAtk, ctx->move);
    bool32 reflect = (sideStatus & SIDE_STATUS_REFLECT) && IsBattlerBattleMovePhysical(ctx->battlerAtk, ctx->move);
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_ICE_SCALES:
        if (IsBattlerBattleMoveSpecial(ctx->battlerAtk, ctx->move))
        {
            modifier =  UQ_4_12(0.5);
            recordAbility = TRUE;
        }
...
static inline uq4_12_t GetAttackerItemsModifier(u32 battlerAtk, uq4_12_t typeEffectivenessModifier, enum HoldEffect holdEffectAtk, u32 isCrit)
{
    u32 metronomeTurns;
...
static inline uq4_12_t GetDefenderItemsModifier(struct DamageContext *ctx)
{
    ...
    case HOLD_EFFECT_GLASS_ARMOR:
        if (!gBattleMons[ctx->battlerDef].volatiles.substitute
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    u32 itemAtk = GetBattlerHoldEffect(ctx->battlerAtk);
    u32 itemDef = GetBattlerHoldEffect(ctx->battlerDef);

    if (mod < UQ_4_12(1.0) && GetBattlerAbility(ctx->battlerAtk) == ABILITY_OMNIPOTENCE) {
        mod = UQ_4_12(1.0);
        if (ctx->updateFlags)
    ...
    else if (ctx->moveType == TYPE_POISON && defType == TYPE_STEEL && ctx->abilityAtk == ABILITY_CHLOROTOXIN && !ctx->isAnticipation) {
        mod = UQ_4_12(2.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilityAtk);
    }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    if (!IsBattlerGrounded(ctx->battlerDef, GetBattlerAbility(ctx->battlerDef), GetBattlerHoldEffect(ctx->battlerDef))
...
bool32 CanMegaEvolve(u32 battler)
{
    ...
    // DO NOT DOUBLE MEGA
    if (HasPartner(battler) && GetBattlerHoldEffect(GetPartnerBattler(battler)) == HOLD_EFFECT_MEGA_STONE 
    && GetBattlerPosition(battler) != B_POSITION_PLAYER_LEFT && GetBattlerPosition(battler) != B_POSITION_PLAYER_RIGHT)
        return FALSE;
...
bool32 TryBattleFormChange(u32 battler, enum FormChanges method)
{
    ...
    if (targetSpecies != currentSpecies && targetSpecies != SPECIES_NONE)
    {
        ...
        // Keep the fainted form's ability for post-hit effects such as contact abilities.
...
static enum DamageCategory SwapDamageCategory(enum DamageCategory category)
{
...
{
    return SwapDamageCategory(GetMoveCategory(move));
}
...
enum DamageCategory GetBattlerBattleMoveCategory(u32 battler, u32 move)
{
    ...
    if (category != DAMAGE_CATEGORY_STATUS
     && gMain.inBattle
     && GetBattlerHoldEffect(battler) == HOLD_EFFECT_STRANGE_AMULET)
        category = SwapDamageCategory(category);
...
{
    return GetBattlerBattleMoveCategory(battler, move) == DAMAGE_CATEGORY_PHYSICAL;
}

bool32 IsBattlerBattleMoveSpecial(u32 battler, u32 move)
{
    return GetBattlerBattleMoveCategory(battler, move) == DAMAGE_CATEGORY_SPECIAL;
}
...
bool32 IsBattlerAffectedByHazards(u32 battler, bool32 toxicSpikes)
{
    ...
    else if (holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS)
    {
        ret = FALSE;
        RecordItemEffectBattle(battler, holdEffect);
    } else if (GetBattlerAbility(battler) == ABILITY_ICE_SCALES) {
        ret = FALSE;
...
bool32 CompareStat(u32 battler, enum Stat statId, u8 cmpTo, u8 cmpKind, enum Ability ability)
{
    ...
    if (ability == ABILITY_CONTRARY || ability == ABILITY_CONTRARIAN)
    {
...
u32 GetAttackerWeather(enum HoldEffect holdEffect, enum Ability ability, u32 weather)
{
    ...
    if (ability == ABILITY_STORMBREAKER)
...
{
    bool8 hasSereneGrace = (battlerAbility == ABILITY_SERENE_GRACE || battlerAbility == ABILITY_MAGIC_WINDS);
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    if ((gBattleMons[battlerDef].volatiles.lockOn && gDisableStructs[battlerDef].battlerWithSureHit == battlerAtk)
     ...
     || (nonVolatileStatus == MOVE_EFFECT_FREEZE_OR_FROSTBITE && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_ICE))
    ...
    // If the target has the ability No Guard and they aren't involved in a Sky Drop or the current move isn't Sky Drop, move hits.
    else if (abilityAtk == ABILITY_LORD_OF_SPACE && (moveEffect != EFFECT_SKY_DROP || gBattleStruct->skyDropTargets[battlerDef] == SKY_DROP_NO_TARGET))
    {
        effect = TRUE;
        ability = ABILITY_LORD_OF_SPACE;
    }

    // If the target has the ability No Guard and they aren't involved in a Sky Drop or the current move isn't Sky Drop, move hits.
    else if (abilityAtk == ABILITY_HYPNOTIST && (moveEffect != EFFECT_SKY_DROP || gBattleStruct->skyDropTargets[battlerDef] == SKY_DROP_NO_TARGET) && gMovesInfo[move].category == DAMAGE_CATEGORY_STATUS)
    {
        effect = TRUE;
    ...
    {
        if (MoveAlwaysHitsInRain(move) && (IsBattlerWeatherAffected(GetBattlerHoldEffect(battlerDef), GetWeather(), B_WEATHER_RAIN) || GetBattlerAbility(battlerAtk) == ABILITY_STORMBREAKER))
            effect = TRUE;
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, enum Ability atkAbility, enum Ability defAbility, enum HoldEffect atkHoldEffect, enum HoldEffect defHoldEffect)
{
    ...
    if (GetAttackerWeather(atkHoldEffect, atkAbility, GetWeather()) & B_WEATHER_SUN && MoveHas50AccuracyInSun(move))
        moveAcc = 50;
    // Check Wonder Skin.
...
{
    if (selectedMove == MOVE_TRICK
     || selectedMove == MOVE_SWITCHEROO
     || selectedMove == MOVE_FLING)
        return FALSE;

    return candidateMove > MOVE_NONE
        && candidateMove < MOVES_COUNT
        && candidateMove != selectedMove
        && GetMoveEffect(candidateMove) != EFFECT_PLACEHOLDER
        && GetMoveType(candidateMove) == GetMoveType(selectedMove)
        && GetMoveCategory(candidateMove) == GetMoveCategory(selectedMove)
        && !IsMoveMetronomeBanned(candidateMove)
        && !IsZMove(candidateMove)
        && !IsMaxMove(candidateMove);
}
...
u32 GetThesaurusMove(u32 selectedMove)
{
    ...
    {
        if (IsMoveValidForThesaurus(selectedMove, move))
        {
            sThesaurusSelectedMove = selectedMove;
            return RandomUniformExcept(RNG_THESAURUS, MOVE_NONE + 1, MOVES_COUNT - 1, IsInvalidThesaurusMove);
        }
    }
...
u32 GetNaturePowerMove(u32 battler)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        move = MOVE_MOONBLAST;
    else if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || GetBattlerAbility(battler) == ABILITY_STORMBREAKER)
