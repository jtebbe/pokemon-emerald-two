...
void FieldGetPlayerInput(struct FieldInput *input, u16 newKeys, u16 heldKeys)
{
    ...
    if(DEBUG_OVERWORLD_MENU && !DEBUG_OVERWORLD_IN_MENU && FlagGet(FLAG_DEBUG_ENABLED))
    {
...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if(input->input_field_1_2 && DEBUG_OVERWORLD_MENU && !DEBUG_OVERWORLD_IN_MENU && FlagGet(FLAG_DEBUG_ENABLED))
    {
        PlaySE(SE_WIN_OPEN);
        FreezeObjectEvents();
        Debug_ShowMainMenu();
