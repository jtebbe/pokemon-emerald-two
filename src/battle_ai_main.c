...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_SUBSTITUTE:
            if (gBattleMons[battlerAtk].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_INFILTRATOR || aiData->items[battlerDef] == ITEM_BURGLARY_KIT)
                ADJUST_SCORE(-8);
        ...
        case EFFECT_PROTECT:
            {
                 ...
                 && protectMethod != PROTECT_CRAFTY_SHIELD) //These moves have infinite usage
                {
                    ...
                    else if (gDisableStructs[battlerAtk].protectUses == 1 && Random() % 100 < 30)
                    {
                        if (isBattle1v1)
                            ADJUST_SCORE(-6);
                        else
                            ADJUST_SCORE(-30); //Don't try double protecting in doubles
                    }
