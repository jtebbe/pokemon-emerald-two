...
#define OW_FLAG_NO_ENCOUNTER        0x1AB  // If this flag is set, wild encounters will be disabled.
...
#define OW_FLAG_NO_COLLISION        FLAG_WALK_ANYWHERE  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
#define OW_FLAG_POKE_RIDER          FLAG_FLY_FROM_POKENAV  // If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
